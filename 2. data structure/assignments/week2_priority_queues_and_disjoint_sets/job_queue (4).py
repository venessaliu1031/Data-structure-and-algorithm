...
def siftDown(i, next_free_time, worker_keys):
    ...
    if i*2+1 < len(next_free_time) and next_free_time[i*2+1] < next_free_time[i]: 
        minChild = i*2+1
    elif i*2+1 < len(next_free_time) and next_free_time[i*2+1] == next_free_time[i]:
        if worker_keys[i*2+1] < worker_keys[i]:
            minChild = i*2+1
    if i*2+2 < len(next_free_time) and next_free_time[i*2+2] < next_free_time[minChild]:
        minChild = i*2+2
    elif i*2+2 < len(next_free_time) and next_free_time[i*2+2] == next_free_time[minChild]:
        if worker_keys[i*2+2] < worker_keys[minChild]:
            minChild = i*2+2
    if minChild != i:
